...
const cardTypesObj = {
    ...
    MasterCard: Constants.cardLabels.MasterCard,
    AmericanExpress: Constants.cardLabels.AmericanExpress,
    //DinersClub: Constants.cardLabels.DinersClub,
...
export default class CardPaymentMethod extends LightningElement {
    ...
    get cardHolderName() {
        ...
        /**
     * Gets or sets the lastname of the card holder.
     *
     * The value of this property is updated in response to user interactions with the control.
     *
     * @type {String}
    ...
    get cardHolderLastName() {
        return this._cardHolderLastName;
    }
    ...
    /**
     * Determines if the card holder last name field is required.
     * @type {Boolean}
     */
     ...
     * @type {Boolean}
     */
    @api cardHolderLastNameRequired = false;
    ...
    /**
     * Determines if the card holder last name field should be hidden.
     * @type {Boolean}
     */
    @api hideCardHolderLastName = false;
    ...
    hasIncompleteCardPaymentFields() {
        const fieldRequiredValueMap = [
            {
                isHiddenAttr: this.hideCardHolderLastName,
                isRequiredAttr: this.cardHolderLastNameRequired,
                value: this.cardHolderLastName,
        ...
        let isInvalidField = fieldRequiredValueMap.find((field) => {
        ...
        if(!isInvalidField && !this.hideExpiryYear ) {
            isInvalidField = !this.checkValidExpiry();
        }
    ...
    checkValidExpiry() {
        const date = new Date();
        let ymNow = '' + date.getFullYear() + (date.getMonth() + 1);
        let ym = this.expiryYear + this.expiryMonth;
        if (ym < ymNow) {
            this._creditCardErrorMessage = this.labels.InvalidExpiryDate;
        }
        return (ym > ymNow);
    }  
    /**
     ...
     * @private
     */
    handleCardHolderLastNameChange(event) {
        this._cardHolderLastName = event.target.value;
        event.target.reportValidity();
    }
     ...
     * @private
     */
    handleCardNumberkeyPressed(event) {
